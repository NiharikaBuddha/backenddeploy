...
async function showLeaderboard(){
    try{
        ...
        inputElement.onclick = async()=>{
                ...
                const userLeaderboardArray= await axios.get("http://16.171.189.212:3000/leaderboard",{headers:{"Authorization": token}});
...
async function showDownloadButton(){
    try{
        ...
        element.onclick = async()=>{
                //e.preventDefault();
                const token = localStorage.getItem('token');
                const downloaded = await axios.get(`http://16.171.189.212:3000/download`,{headers:{"Authorization": token}});
...
window.addEventListener("DOMContentLoaded", async()=>{
    try{
       ...
       if(isPremium === true){
        ...
        const response = await axios.get("http://16.171.189.212:3000/show-expenses",{headers: {"Authorization": token}})
        console.log(response.data.allExpense);
        console.log(response.data.allExpense.length);
        for(var i=0; i<response.data.allExpense.length;i++){
            showBrowser(response.data.allExpense[i]);
...
async function pagination(){
    try{
        ...
        const result = await axios.get(`http://16.171.189.212:3000/pagination?page=${1}&pagesize=${5}`,{headers: {"Authorization": token}});
        ...
        for(let i=0;i<totalPage;i++){
            ...
            button.onclick = async()=>{
                ...
                const ress = await axios.get(`http://16.171.189.212:3000/pagination?page=${page}&pagesize=${totalPagesize}`,{headers: {"Authorization": token}});
...
async function deleteExpense(key){
    try{
        ...
        const token = localStorage.getItem('token');
        await axios.delete(`http://16.171.189.212:3000/delete-expenses/${key}`,{headers: {"Authorization": token}})
...
button.addEventListener("click", async(e)=>{
    ...
    console.log(myObj);
    const token = localStorage.getItem('token');
    const addExpense = await axios.post("http://16.171.189.212:3000/add-expenses",myObj,{headers:{'Authorization': token}})
...
document.getElementById("premium").onclick = async function(e){
    try{
    const token = localStorage.getItem('token');
    const response = await axios.get('http://16.171.189.212:3000/premium-membership',{headers:{"Authorization": token}});
    console.log(response);
    var options = {
        "key": response.data.key_id,
        "order_id": response.data.order.id,
        "handler": async function(response){
            let result = await axios.post("http://16.171.189.212:3000/update-transaction-status",{
    ...
    rzp1.on("payment.failed", async()=>{
        ...
        let failed = await axios.post("http://16.171.189.212:3000/update-transaction-status",{
            order_id: key,
            payment_id: null
        },{headers: {"Authorization": token}})
